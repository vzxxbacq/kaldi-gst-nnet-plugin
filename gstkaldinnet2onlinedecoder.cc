...
static void gst_kaldinnet2onlinedecoder_final_result(
    ...
    if (full_final_result.nbest_results.size() > 0) {
        ...
        *num_words = full_final_result.nbest_results[0].words.size();
        ...
        if (hyp_length > 0) {
            // push partial result to pad.
            json_t *root = json_object();
            // 1 -> final for segment.
            json_object_set_new( root, "status", json_integer(1));
            json_object_set_new( root, "result", json_string(best_transcript.c_str()));
            char* result = json_dumps(root, JSON_COMPACT);
            guint length = strlen(result);

            GstBuffer *buffer = gst_buffer_new_and_alloc(length);
            gst_buffer_fill(buffer, 0, result, length);
...
static void gst_kaldinnet2onlinedecoder_partial_result(
    ...
    if (transcript.length() > 0) {
        ...
        // 0 -> partial.
        json_object_set_new( root, "status", json_integer(0));
        json_object_set_new( root, "result", json_string(transcript.c_str()));
        char* result = json_dumps(root, JSON_COMPACT);
        guint length = strlen(result);
...
gst_kaldinnet2onlinedecoder_query (GstPad *pad, GstObject * parent, GstQuery * query) {
    ...
    // setting sample rate by feature conf.
...
GST_PLUGIN_DEFINE(GST_VERSION_MAJOR, GST_VERSION_MINOR, kaldinnet2onlinedecoder,
                  ...
                  kaldi::kaldinnet2onlinedecoder_init, VERSION, "unknown", "GStreamer",
                  "http://gstreamer.net/")