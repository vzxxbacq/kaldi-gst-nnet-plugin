...
struct _GstModelObjects {
  TransitionModel *trans_model;
  nnet2::AmNnet *am_nnet2;
  nnet3::AmNnetSimple *am_nnet3;
};

typedef std::string MODEL_MAP_KEY;
typedef _GstModelObjects* MODEL_MAP_VALUE;
typedef std::map<MODEL_MAP_KEY, MODEL_MAP_VALUE> MODEL_MAP_TYPE;

typedef std::string FST_MAP_KEY;
typedef fst::Fst<fst::StdArc>* FST_MAP_VALUE;
typedef std::map<FST_MAP_KEY, FST_MAP_VALUE> FST_MAP_TYPE;

struct _Gstkaldinnet2onlinedecoder {
  static FST_MAP_TYPE fst_map;
  static MODEL_MAP_TYPE model_map;
