...
#include "gstkaldinnet2onlinedecoder-shared.h"
...
MODEL_MAP_TYPE _Gstkaldinnet2onlinedecoder::model_map;
  ...
  if (G_VALUE_HOLDS_STRING(value)) {
    ...
    if (strcmp(str, "") != 0) {
       try {
        ...
        // Optimization to share the model objects between threads within the process
        MODEL_MAP_TYPE::iterator it = _Gstkaldinnet2onlinedecoder::model_map.find(std::string(str));
        if (it != _Gstkaldinnet2onlinedecoder::model_map.end()) {
            fprintf(stderr, ">>>>>>>>>> *model* is already loaded at pointer %p \n", it->second);
            _GstModelObjects* model_objects = it->second;
            filter->trans_model = model_objects->trans_model;
            filter->am_nnet2 = model_objects->am_nnet2;
            filter->am_nnet3 = model_objects->am_nnet3;
        } else {
            ...
            } else if (filter->nnet_mode == NNET3) {
              filter->am_nnet3->Read(ki.Stream(), binary);
            }

            _GstModelObjects* model_objects = new _GstModelObjects();
            model_objects->trans_model = filter->trans_model;
            model_objects->am_nnet2 = filter->am_nnet2;
            model_objects->am_nnet3 = filter->am_nnet3;
            _Gstkaldinnet2onlinedecoder::model_map[std::string(str)] = model_objects;
        }
        ...
        if (filter->nnet_mode == NNET3) {
            ...
            filter->decodable_info_nnet3 = new nnet3::DecodableNnetSimpleLoopedInfo(*(filter->nnet3_decodable_opts), filter->am_nnet3);
        }
...
FST_MAP_TYPE _Gstkaldinnet2onlinedecoder::fst_map;
  ...
  if (G_VALUE_HOLDS_STRING(value)) {
    ...
    if (strcmp(str, "") != 0) {
      try {
        ...
        fst::Fst<fst::StdArc> *new_decode_fst;

        // Optimization to share the fst object between threads within the process
        FST_MAP_TYPE::iterator it = _Gstkaldinnet2onlinedecoder::fst_map.find(std::string(str));
        if (it != _Gstkaldinnet2onlinedecoder::fst_map.end()) {
          fprintf(stderr, ">>>>>>>>>> *fst* is already loaded at pointer %p \n", it->second);
          new_decode_fst = it->second;
        } else {
          new_decode_fst = fst::ReadFstKaldiGeneric(str);
          _Gstkaldinnet2onlinedecoder::fst_map[std::string(str)] = new_decode_fst;
        }
